...
def make_device_info():
    ...
    d1_values = ['P2','8','4','true','09:30','10:00','true','18:00']
...
# not possible to use unless connected to a remote device that has
# provided a csv file

# currently retunrs a template device_info dataframe

def get_device_info():
    
    #device_info = read_csv
    #device_info_df = pd.DataFrame(device_info)
    #device_info_df = pd.DataFrame(device_info_df)
    
    df_keys = ['device_name','power','on_period','use_OFF','s_off','e_off','use_deadline','deadline']
    df_values = ['EV', 4, 3, 'true', '1970-01-01T15:00:00.000Z', '1970-01-01T17:00:00.000Z', 'true', '1970-01-01T17:00:00.000Z']
    device_data = [df_keys, df_values]
    device_df_wrong = pd.DataFrame(device_data)
    device_df = device_df_wrong.transpose()
...
def use_op1():
    ...
    print('use op2')
    print('Start device at: ' + min_time.to_string(index=False))
...
def use_op3():
    ...
    # make first half of ci and pr
    cut1_pr = pr_df_crop.iloc[:int(deadline_int[1:-1])-int(on_period),:]
    cut1_ci = ci_df_crop.iloc[:int(deadline_int[1:-1])-int(on_period),:]
...
def use_op4():
    ...
    # Option 3
        ...
        if cut1_okay == False:
            ...
            # Print time at which Z is minimum
            print('use op5')
            print('run 3 2 split run (cropped)')
            print('Start device at: ' + start_time_1)
            print('End device at: ' + end_time)
